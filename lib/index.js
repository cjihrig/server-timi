...
function onPreResponse (request, h) {
  // It's possible that request.timings hasn't been setup properly if an
  // onRequest extension threw. The other extension points should be fine
  // (unless the user tampered with request.timings intentionally).
  if (!(request.timings instanceof Timings)) {
    return h.continue;
  }
